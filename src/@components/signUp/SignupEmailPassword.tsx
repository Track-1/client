...
export default function SignupEmailPassword(props: SetPropsType) {
  ...
  const PostAuthMail = useMutation(authEmail, {
    onSuccess: (data) => {
      ...
      alert("Authentication code sent. Please check your mailbox. If you haven't received the mail, please check your spam mail box.\n인증코드를 보냈습니다. 메일함을 확인해주세요. 메일을 받지 못하셨다면 스팸메일함을 확인해주세요.");
    },
    onError: (error: any) => {
      console.log(error);
